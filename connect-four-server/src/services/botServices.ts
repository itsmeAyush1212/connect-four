...
export class BotService {
  ...
  getBestMove(game: GameState, botColor: 'R' | 'Y'): number {
  ...
  private findWinningMove(board: CellValue[][], color: 'R' | 'Y', cols: number): number {
  ...
  private findStrategicMove(board: CellValue[][], color: 'R' | 'Y'): number {
  ...
  private findCenterMove(board: CellValue[][]): number {
  ...
  private findAnyValidMove(board: CellValue[][]): number {
    for (let col = 0; col < this.COLS; col++) {
      if (this.isValidMove(board, col)) {
        return col;
      }
  ...
  private getLowestRow(board: CellValue[][], col: number): number {
  ...
  private countThreats(board: CellValue[][], row: number, col: number, color: 'R' | 'Y'): number {
    let count = 0;
    const directions = [
      [0, 1], // Horizontal
      [1, 0], // Vertical
    ...
    for (const [rowDelta, colDelta] of directions) {
      ...
      if (lineCount >= 3) count += 2;
      else if (lineCount >= 2) count += 1;
    }
